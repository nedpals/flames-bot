export default function spin(idx_number: number, debug: boolean = false): string {
    const flames_obj = {
        "F": "friends",
        "L": "lovers",
        "A": "anger",
    ...
    const arr: string[] = Object.keys(flames_obj);
    ...
    if (idx_number === 0) {
        ...
        [...Array(groups).keys()].forEach((group, g_i, gs_arr) => {
            [...arr.keys()].forEach((i, _, g_arr) => {
                if (debug) {console.log(`Group: ${group}`, i, arr[i]);}
        ...
        if (remainder !== 0) {
            [...Array(remainder).keys()].forEach((i, _, r_arr) => {
                if (debug) {console.log(`Remainder`, i, arr[i]);}
